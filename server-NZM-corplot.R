...
NZM_RNAseqdata_corplot_df <- eventReactive(input$NZM_make_corplot, {
  NZM_RNAseqdata_corplot %>% 
    filter(symbols %in% c(isolate({input$NZM_corplot_gene1}), isolate({input$NZM_corplot_gene2}))) %>% 
...
NZM_plotly_corplot <- eventReactive(input$NZM_make_corplot ,{
  NZM_RNAseqdata_corplot_df() %>% 
    filter(batch %in% isolate({input$NZM_corplot_batch})) %>% 
    plot_ly(x = ~get(isolate({input$NZM_corplot_gene1})), y = ~get(isolate({input$NZM_corplot_gene2})), symbol = ~batch, hoverinfo = "text",
...
NZM_corvalues <- eventReactive(input$NZM_make_corplot, {
  df <-  NZM_RNAseqdata_corplot_df() %>% filter(batch %in% input$NZM_corplot_batch)
...
output$DT_NZM_corplot_df <- DT::renderDT(NZM_RNAseqdata_corplot_df() %>% 
...
NZM_RNAseqdata_corplot_samples_df <- eventReactive(input$NZM_make_corplot_samples, {
  NZM_RNAseqdata_corplot %>% 
    filter(symbols %in% c(isolate({input$NZM_corplot_gene3}), isolate({input$NZM_corplot_gene4}))) %>% 
    data.frame(row.names = 1) %>% 
    t %>% 
    as_tibble(rownames = "samples") %>% 
    mutate(batch = ifelse(samples %in%  batch1_samples, "batch1", "batch2"))
...
NZM_plotly_corplot_samples <- eventReactive(input$NZM_make_corplot_samples ,{
    ...
    plot_ly(x = ~get(isolate({input$NZM_corplot_gene3})), y = ~get(isolate({input$NZM_corplot_gene4})), hoverinfo = "text",
            text = ~paste0(input$NZM_corplot_gene3, ": ", get(isolate({input$NZM_corplot_gene3})), "<br>",
                           input$NZM_corplot_gene4, ": ", get(isolate({input$NZM_corplot_gene4})), "<br>",
                           'sample: ', samples)) %>% 
    layout(xaxis = list(title = paste(isolate({input$NZM_corplot_gene3}), "(log2 TPM)")), 
           yaxis = list(title = paste(isolate({input$NZM_corplot_gene4}), "(log2 TPM)"))
           ) %>% 
...
NZM_corvalues_samples <- eventReactive(input$NZM_make_corplot_samples, {
  ...
  corvalue <- cor(df[[input$NZM_corplot_gene3]],  df[[input$NZM_corplot_gene4]])
  corpvalue <- cor.test(df[[input$NZM_corplot_gene3]],  df[[input$NZM_corplot_gene4]])$p.value
  paste("Pearson correlation value =",corvalue ,"pvalue =", corpvalue)
}, 
ignoreNULL = FALSE,
ignoreInit = TRUE)
